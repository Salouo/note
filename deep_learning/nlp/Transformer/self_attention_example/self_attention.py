...
# Divide QKT with the square root of the key vector dimension
d_k = torch.tensor(K.shape[1], dtype=torch.float32, device=device)
attention = torch.softmax(QKT / torch.sqrt(d_k), dim=1)

# Finally, compute the output matrix
# Notice that, it keeps the identical shape of the input matrix.
output_attention = attention @ V   # shape: (5, 512)
